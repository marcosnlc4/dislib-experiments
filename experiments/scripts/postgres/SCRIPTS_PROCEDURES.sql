...
DECLARE
	...
	arr_resource_data text[] := '{
									{MARENOSTRUM_1,1,64,4,512000000000,64000000000},
									{MARENOSTRUM_2,2,128,8,512000000000,64000000000}
								 }';
	...
	arr_dataset_data text[] := '{
									{S_KMEANS_10,64000000000,FLOAT64,8,800000000,8000000,100,170,0.0,0.0},
									{S_KMEANS_20,57600000000,FLOAT64,8,1600000000,16000000,100,170,0.0,0.0},
									{S_KMEANS_30,51200000000,FLOAT64,8,2400000000,24000000,100,170,0.0,0.0},
									{S_KMEANS_40,44800000000,FLOAT64,8,3200000000,32000000,100,170,0.0,0.0},
							  		{S_KMEANS_50,38400000000,FLOAT64,8,4000000000,40000000,100,170,0.0,0.0},
									{S_KMEANS_60,32000000000,FLOAT64,8,4800000000,48000000,100,170,0.0,0.0},
									{S_KMEANS_70,25600000000,FLOAT64,8,5600000000,56000000,100,170,0.0,0.0},
									{S_KMEANS_80,19200000000,FLOAT64,8,6400000000,64000000,100,170,0.0,0.0},
							  		{S_KMEANS_90,12800000000,FLOAT64,8,7200000000,72000000,100,170,0.0,0.0},
									{S_KMEANS_100,6400000000,FLOAT64,8,8000000000,80000000,100,170,0.0,0.0}
								}';
...
BEGIN
	...
	THEN
		...
		IF (param_grid_row_dimension = 'MAXCORES')
		THEN
			grid_row_dimension := (SELECT MAX((NR_NODES-1) * NR_COMPUTING_UNITS_CPU) AS NR_TOTAL_CORES FROM RESOURCE);
			grid_column_dimension := param_grid_column_dimension;
		END IF;
	...
	THEN
		...
		-- SELECT AVAILABLE RESOURCES
		arr_id_resource := ARRAY(SELECT DISTINCT ID_RESOURCE FROM RESOURCE WHERE (ID_RESOURCE = 3 OR ID_RESOURCE BETWEEN 11 AND 17) ORDER BY ID_RESOURCE);
		arr_id_dataset := ARRAY(SELECT DISTINCT ID_DATASET FROM DATASET WHERE DS_DATASET IN ('S_1MB_1','S_10MB_1','S_100MB_1','S_1GB_1','S_10GB_1','S_100GB_1') ORDER BY ID_DATASET);
...
-- UPDATE_INTRA_TIME_FULL_FUNC
CREATE OR REPLACE PROCEDURE UPDATE_INTRA_TIME_FULL_FUNC()
LANGUAGE plpgsql
AS $BODY$
#variable_conflict use_column
DECLARE
	arr_id_experiment bigint[];
	id_experiment_iterator bigint;
...
BEGIN
	...
	arr_id_experiment := ARRAY(SELECT
									ID_EXPERIMENT
									from
									EXPERIMENT_RAW BB
									where
									...
									(
										...
										ROUND(cast(A.VL_INTRA_TASK_EXECUTION_TIME_FULL_FUNC/(A.VL_INTRA_TASK_EXECUTION_TIME_DEVICE_FUNC+A.VL_COMMUNICATION_TIME_1 + A.VL_COMMUNICATION_TIME_2+A.VL_ADDITIONAL_TIME_1 + A.VL_ADDITIONAL_TIME_2) as numeric),2) AS FACTOR,
										A.VL_INTRA_TASK_EXECUTION_TIME_FULL_FUNC,
										A.VL_INTRA_TASK_EXECUTION_TIME_DEVICE_FUNC+A.VL_COMMUNICATION_TIME_1 + A.VL_COMMUNICATION_TIME_2+A.VL_ADDITIONAL_TIME_1 + A.VL_ADDITIONAL_TIME_2 AS V2_VL_INTRA_TASK_EXECUTION_TIME_FULL_FUNC,
										...
										WHERE
										ROUND(cast(A.VL_INTRA_TASK_EXECUTION_TIME_FULL_FUNC/(A.VL_INTRA_TASK_EXECUTION_TIME_DEVICE_FUNC+A.VL_COMMUNICATION_TIME_1 + A.VL_COMMUNICATION_TIME_2+A.VL_ADDITIONAL_TIME_1 + A.VL_ADDITIONAL_TIME_2) as numeric),2) is not null
										AND ROUND(cast(A.VL_INTRA_TASK_EXECUTION_TIME_FULL_FUNC/(A.VL_INTRA_TASK_EXECUTION_TIME_DEVICE_FUNC+A.VL_COMMUNICATION_TIME_1 + A.VL_COMMUNICATION_TIME_2+A.VL_ADDITIONAL_TIME_1 + A.VL_ADDITIONAL_TIME_2) as numeric),2) <> 1.00
										) xx
									)
		);
		
	-- FOR EACH ID_EXPERIMENT
	FOREACH id_experiment_iterator IN ARRAY arr_id_experiment
	LOOP
		UPDATE EXPERIMENT_RAW
		SET VL_INTRA_TASK_EXECUTION_TIME_FULL_FUNC = (SELECT AA.VL_INTRA_TASK_EXECUTION_TIME_DEVICE_FUNC+AA.VL_COMMUNICATION_TIME_1+AA.VL_COMMUNICATION_TIME_2+AA.VL_ADDITIONAL_TIME_1+AA.VL_ADDITIONAL_TIME_2 FROM EXPERIMENT_RAW AA WHERE AA.ID_EXPERIMENT = id_experiment_iterator)
		WHERE
		ID_EXPERIMENT = id_experiment_iterator;
	END LOOP;
