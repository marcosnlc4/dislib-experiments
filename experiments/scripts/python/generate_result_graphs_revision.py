...
def generate_graph(df, dst_path_figs, ds_algorithm, ds_resource, nr_iterations, mode):
    ...
    if mode == 1:
        ...
        # print(df_filtered['ds_parameter_type'])
        ...
        # df1_cpu = df_filtered_mean_left_top_cpu.sort_values('vl_block_row_dimension', inplace=True)
        # df1_gpu = df_filtered_mean_left_top_gpu.sort_values('vl_block_row_dimension', inplace=True)

        # df2_cpu = df_filtered_mean_right_top_cpu.sort_values('vl_block_row_dimension', inplace=True)
        # df2_gpu = df_filtered_mean_right_top_gpu.sort_values('vl_block_row_dimension', inplace=True)

        # df3_cpu = df_filtered_mean_left_bottom_cpu.sort_values('vl_block_row_dimension', inplace=True)
        # df3_gpu = df_filtered_mean_left_bottom_gpu.sort_values('vl_block_row_dimension', inplace=True)

        # df4_cpu = df_filtered_mean_right_bottom_cpu.sort_values('vl_block_row_dimension', inplace=True)
        # df4_gpu = df_filtered_mean_right_bottom_gpu.sort_values('vl_block_row_dimension', inplace=True)

        fig, axs = plt.subplots(2, 2, figsize=(12, 8), sharey='row', sharex='col')

        # Plot the first chart (top-left - Bar chart)
        axs[0, 0].plot(df1_cpu[x_value],df1_cpu['vl_inter_task_execution_time'], color='C1', linestyle = 'dotted', label='CPU', zorder=3, linewidth=2.5)
        axs[0, 0].plot(df1_gpu[x_value],df1_gpu['vl_inter_task_execution_time'], color='C1', linestyle = 'solid', label='GPU', zorder=3, linewidth=2.5)
        axs[0, 0].legend(loc='upper left', frameon=False, labelspacing=0.01, ncol=2, borderpad=0.1)
        axs[0, 0].set_ylabel('P. Tasks Average Time (s)')  # Add y-axis label
        axs[0, 0].set_ylim([0, 265])
        axs[0, 0].grid(zorder=0,axis='y')
        axs[0, 0].set_title('Local disk, task generation order', pad=10)

        # Plot the second chart (top-right - Bar chart)
        axs[0, 1].plot(df2_cpu[x_value],df2_cpu['vl_inter_task_execution_time'], color='C1', linestyle = 'dotted', zorder=3, linewidth=2.5)
        axs[0, 1].plot(df2_gpu[x_value],df2_gpu['vl_inter_task_execution_time'], color='C1', linestyle = 'solid', zorder=3, linewidth=2.5)
        axs[0, 1].legend(loc='upper left', frameon=False, labelspacing=0.01, ncol=2, borderpad=0.1)
        axs[0, 1].set_ylim([0, 265])
        axs[0, 1].grid(zorder=0,axis='y')
        axs[0, 1].set_title('Local disk, data locality', pad=10)

        # Plot the third chart (bottom-left - Line chart)
        axs[1, 0].plot(df3_cpu[x_value],df3_cpu['vl_inter_task_execution_time'], color='C1', linestyle = 'dotted', zorder=3, linewidth=2.5)
        axs[1, 0].plot(df3_gpu[x_value],df3_gpu['vl_inter_task_execution_time'], color='C1', linestyle = 'solid', zorder=3, linewidth=2.5)
        axs[1, 0].set_xlabel('Block size MB (Grid Dimension)')
        axs[1, 0].set_ylabel('P. Tasks Average Time (s)')  # Add y-axis label
        axs[1, 0].set_ylim([0, 265])
        axs[1, 0].tick_params(axis='x', labelrotation = 35)
        axs[1, 0].grid(zorder=0,axis='y')
        axs[1, 0].set_title('Shared disk, task generation order', pad=10)
        
        
        # Plot the fourth chart (bottom-center - Line chart)
        axs[1, 1].plot(df4_cpu[x_value],df4_cpu['vl_inter_task_execution_time'], color='C1', linestyle = 'dotted', zorder=3, linewidth=2.5)
        axs[1, 1].plot(df4_gpu[x_value],df4_gpu['vl_inter_task_execution_time'], color='C1', linestyle = 'solid', zorder=3, linewidth=2.5)
        axs[1, 1].set_xlabel('Block size MB (Grid Dimension)')
        axs[1, 1].set_ylim([0, 265])
        axs[1, 1].tick_params(axis='x', labelrotation = 35)
        axs[1, 1].grid(zorder=0,axis='y')
        axs[1, 1].set_title('Shared disk, data locality', pad=10)
    ...
    #     plt.savefig(dst_path_figs+'mode_'+str(mode)+'_overview_gpu_speedup_times_'+ds_algorithm+'_'+ds_resource+'_nr_it_'+str(nr_iterations)+'.png',bbox_inches='tight',dpi=100)
        ...
        # K-means Speedup 
        # 0 skewed
        df_filtered_left_top = df_filtered[(df_filtered.device_skewness=='GPU NOT SKEWED')]
        # 50 skewed
        df_filtered_right_top = df_filtered[(df_filtered.device_skewness=='GPU SKEWED')]
        
        # K-means Time
        # 0 skewed
        df_filtered_left_bottom = df_filtered[(df_filtered.device_skewness=='GPU NOT SKEWED')]
        # 50 skewed
        df_filtered_right_bottom = df_filtered[(df_filtered.device_skewness=='GPU SKEWED')]

        left_title = 'Dataset 0% skewed'
        right_title = 'Dataset 50% skewed'
        ...
        # Plot the first chart (top-left - Bar chart)
        # axs[0, 0].bar(df1[x_value],df1['speedup_gpu_intra_task_execution_time_full_func'], color='C0', alpha = 0.25, label='Usr. Code')
        axs[0, 0].bar(df1[x_value],df1['speedup_gpu_intra_task_execution_time_full_func'], color='C0', alpha = 0.25)
        axs[0, 0].legend(loc=(-0.000,0.99), frameon=False, labelspacing=0.01, ncol=3, borderpad=0.1)
        axs[0, 0].set_ylabel('GPU Speedup over CPU')  # Add y-axis label
        ...
        # Plot the third chart (bottom-left - Line chart)
        # axs[1, 0].plot(df3[x], df3['vl_intra_task_execution_time_device_func_cpu'], color='C2', linestyle = 'dotted', label='P. Frac.\nCPU', linewidth=2.5)
        # axs[1, 0].plot(df3[x], df3['vl_additional_time_cpu'], color='C8', linestyle = 'dotted', label='S. Frac.\nGPU', linewidth=2.5)
        # axs[1, 0].plot(df3[x], df3['vl_intra_task_execution_time_device_func_gpu'], color='C2', linestyle = 'solid', label='P. Frac.\nGPU', linewidth=2.5)
        # axs[1, 0].plot(df3[x], df3['vl_additional_time_gpu'], color='C8', linestyle = 'solid', label='S. Frac.\nGPU', linewidth=2.5)
        # axs[1, 0].plot(df3[x], df3['vl_communication_time_gpu'], color='C4', linestyle = 'solid', label='CPU-GPU\nComm.', linewidth=2.5)
        axs[1, 0].plot(df3[x], df3['vl_intra_task_execution_time_device_func_cpu'], color='C2', linestyle = 'dotted', linewidth=2.5)
        axs[1, 0].plot(df3[x], df3['vl_additional_time_cpu'], color='C8', linestyle = 'dotted', linewidth=2.5)
        axs[1, 0].plot(df3[x], df3['vl_intra_task_execution_time_device_func_gpu'], color='C2', linestyle = 'solid', linewidth=2.5)
        axs[1, 0].plot(df3[x], df3['vl_additional_time_gpu'], color='C8', linestyle = 'solid', linewidth=2.5)
        axs[1, 0].plot(df3[x], df3['vl_communication_time_gpu'], color='C4', linestyle = 'solid', linewidth=2.5)
        axs[1, 0].legend(loc=(-0.000,0.99), frameon=False, labelspacing=0.01, ncol=5, borderpad=0.1)
        axs[1, 0].set_xlabel('Block size MB')  # Add x-axis label
        axs[1, 0].set_ylabel('Average Time per Task (s)')  # Add y-axis label
        axs[1, 0].set_yscale('log')
        ...
        # Plot the fourth chart (bottom-center - Line chart)
        # axs[1, 1].plot(df4[x], df4['vl_intra_task_execution_time_device_func_cpu'], color='C2', linestyle = 'dotted', label='P. Frac. CPU', linewidth=2.5)
        # axs[1, 1].plot(df4[x], df4['vl_additional_time_cpu'], color='C8', linestyle = 'dotted', label='S. Frac. GPU', linewidth=2.5)
        # axs[1, 1].plot(df4[x], df4['vl_intra_task_execution_time_device_func_gpu'], color='C2', linestyle = 'solid', label='P. Frac. GPU', linewidth=2.5)
        # axs[1, 1].plot(df4[x], df4['vl_additional_time_gpu'], color='C8', linestyle = 'solid', label='S. Frac. GPU', linewidth=2.5)
        # axs[1, 1].plot(df4[x], df4['vl_communication_time_gpu'], color='C4', linestyle = 'solid', label='CPU-GPU Comm.', linewidth=2.5)
        axs[1, 1].plot(df4[x], df4['vl_intra_task_execution_time_device_func_cpu'], color='C2', linestyle = 'dotted', linewidth=2.5)
        axs[1, 1].plot(df4[x], df4['vl_additional_time_cpu'], color='C8', linestyle = 'dotted', linewidth=2.5)
        axs[1, 1].plot(df4[x], df4['vl_intra_task_execution_time_device_func_gpu'], color='C2', linestyle = 'solid', linewidth=2.5)
        axs[1, 1].plot(df4[x], df4['vl_additional_time_gpu'], color='C8', linestyle = 'solid', linewidth=2.5)
        axs[1, 1].plot(df4[x], df4['vl_communication_time_gpu'], color='C4', linestyle = 'solid', linewidth=2.5)
        axs[1, 1].set_xlabel('Block size MB')  # Add x-axis label
        axs[1, 1].set_yscale('log')
        axs[1, 1].set_ylim([1e-3, 1e1])
        axs[1, 1].tick_params(axis='x', labelrotation = 30)
        axs[1, 1].grid(zorder=0,axis='y')
        ...
        #EXAMPLE CHART (USED AS MODEL)
        # # Sample data as pandas dataframes
        # x1 = np.linspace(0, 10, 10)
        # x2 = np.linspace(5, 15, 10)

        # df1 = pd.DataFrame({'x': x1, 'y1': np.sin(x1), 'y2': np.cos(x1)})
        # df2 = pd.DataFrame({'x': x2, 'y3': np.tan(x2)})
        # df3 = pd.DataFrame({'x': x1, 'y1': np.sin(x1), 'y2': np.cos(x1)})
        # df4 = pd.DataFrame({'x': x2, 'y4': np.exp(-x2)})

        # # Create a figure and subplots
        # fig, axs = plt.subplots(2, 2, figsize=(10, 8), sharey='row')

        # # Plot the first chart (top-left - Bar chart)
        # axs[0, 0].bar(df1['x'], df1['y1'], label='sin(x)')
        # axs[0, 0].bar(df1['x'], df1['y2'], label='cos(x)')
        # axs[0, 0].set_title('Chart 1')
        # axs[0, 0].legend()
        # axs[0, 0].set_ylabel('Amplitude')  # Add y-axis label

        # # Plot the second chart (top-right - Bar chart)
        # axs[0, 1].bar(df2['x'], df2['y3'], label='tan(x)')
        # axs[0, 1].set_title('Chart 2')
        # axs[0, 1].legend()

        # # Plot the third chart (bottom-left - Line chart)
        # axs[1, 0].plot(df3['x'], df3['y1'], label='sin(x)')
        # axs[1, 0].plot(df3['x'], df3['y2'], label='cos(x)')
        # axs[1, 0].set_title('Chart 3')
        # axs[1, 0].legend()
        # axs[1, 0].set_xlabel('Time')  # Add x-axis label
        # axs[1, 0].set_ylabel('Amplitude')  # Add y-axis label

        # # Plot the fourth chart (bottom-right - Line chart)
        # axs[1, 1].plot(df4['x'], df4['y4'], label='exp(-x)')
        # axs[1, 1].set_title('Chart 4')
        # axs[1, 1].legend()
        # axs[1, 1].set_xlabel('Time')  # Add x-axis label
        ...
        # df1 = pd.DataFrame({'x': x, 'y1': np.sin(x), 'y2': np.cos(x)})
        # df2 = pd.DataFrame({'x': x, 'y3': np.tan(x)})
        # df3 = pd.DataFrame({'x': x, 'y1': np.sin(x), 'y2': np.cos(x)})
        # df4 = pd.DataFrame({'x': x, 'y4': np.exp(-x)})
